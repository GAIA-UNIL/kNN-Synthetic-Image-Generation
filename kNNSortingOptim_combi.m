function sortedDate = kNNSortingOptim_combi(sortedDates,Tmin_LongW,Tmax_LongW,Pre_LongW,doyW,spemW,helW,metricKNN,nbImages,useDOY,saveOptimPrep,inputDir)
...
for qd = 1:totQDates
    ...
    if metricKNN == 5
        ...
        if useDOY
            doyDist = squeeze(cat(3, distCell{:, 3})) * doyW;
        end
    else 
        if useDOY
            doyDist = squeeze(cat(3, distCell{:, 2})) * doyW;
        end
    end

    totalDistance = sum(D1_weighted + D2_weighted, 2) + doyDist;
